...
export const adaptTransaction: TransactionAdapter = async (rec): Promise<Transaction> => {
 ...
 * @throws {AdapterError}
 */
export type TransferAdapter = (record: ParsedRecord) => Promise<Transfer>;

export const adaptTransfer: TransferAdapter = async (rec): Promise<Transfer> => {
  if (!rec.isTransfer) throw new Error('Transaction adaption is not supported.');
  ...
  return {
    ...
    // eslint-disable-next-line no-underscore-dangle
    labelsID: labelDocuments.map((document) => document._id),
    counterparty: rec.payee.trim() !== '' ? rec.payee : undefined,
  };
};
